    ...
    <div class="container mx-auto px-4 sm:px-6">
      <div class="max-w-4xl mx-auto text-center">
        <h2 class="text-3xl sm:text-4xl md:text-5xl text-primary-dark mb-4 md:mb-5 font-normal tracking-wide" style="font-family: 'Italianno', cursive; font-weight: 400; -webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale;">
          {{ heading || 'Reservierung & Lieferung' }}
        </h2>
        ...
        <p class="text-sm sm:text-base md:text-lg text-primary-dark/80 leading-relaxed max-w-3xl mx-auto tracking-wide font-light px-2">
          {{ description || 'Reservieren Sie Ihren Tisch für ein unvergessliches kulinarisches Erlebnis oder genießen Sie unsere authentische italienische Küche direkt an Ihrer Haustür. Wir bringen Ihnen den Geschmack Italiens, ob Sie bei uns speisen oder in den eigenen vier Wänden.' }}
        </p>
      </div>
    </div>
  </section>
...
const heading = ref('Reservierung & Lieferung')
const description = ref('Reservieren Sie Ihren Tisch für ein unvergessliches kulinarisches Erlebnis oder genießen Sie unsere authentische italienische Küche direkt an Ihrer Haustür. Wir bringen Ihnen den Geschmack Italiens, ob Sie bei uns speisen oder in den eigenen vier Wänden.')

// Load divider content from API
const loadDividerContent = async () => {
  try {
    const response = await fetch(`${API_URL}/content/divider-reservations`)
