    ...
    <div class="container mx-auto px-4 sm:px-6">
      <div class="max-w-4xl mx-auto text-center">
        ...
        <p class="text-sm sm:text-base md:text-lg text-primary-dark/80 leading-relaxed max-w-3xl mx-auto tracking-wide font-light px-2">
          {{ description || 'Hören Sie von unseren geschätzten Kunden über ihre kulinarischen Erfahrungen in der Pizzeria Adria' }}
        </p>
      </div>
    </div>
  </section>
...
const heading = ref('Was unsere Gäste sagen')
const description = ref('Hören Sie von unseren geschätzten Kunden über ihre kulinarischen Erfahrungen in der Pizzeria Adria')

// Load divider content from API
const loadDividerContent = async () => {
  try {
    const response = await fetch(`${API_URL}/content/divider-testimonials`)
